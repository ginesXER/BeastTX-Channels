...
internal static class Program_new_notworking
{
    // ---------------- CONFIG ----------------
    ...
    private const int PING_PERIOD_MS = 500;
    private const int TCP_RETRY_MS = 500;
    ...
    // Watchdog flags (set by RX thread or write failures)
    private static volatile bool _linkBroken = false;

    private static void Main_new()
    {
        // Open HID once (keep it open across reconnects)
        ...
        var hidBuf = new byte[64];

        while (true)
        {
            // 1) Ping gate
            WaitPing(MAVLINK_HOST);

            // 2) TCP connect gate
            using var tcp = ConnectTcpWithRetry(MAVLINK_HOST, MAVLINK_PORT);
            tcp.NoDelay = true;
            ...
            // 3) MAVLink gate (heartbeat). Only after TCP is up.
            try
            {
                WaitHeartbeat(stream); // blocks until HEARTBEAT seen, throws if stream breaks
                Console.WriteLine($"[OK] MAVLink heartbeat. Target sys/comp = {_targetSys}/{_targetComp}");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[MAVLINK GATE FAIL] {ex.GetType().Name}: {ex.Message}");
                continue; // go back to ping->tcp->mavlink
            }

            // 4) Start RX drain thread to detect broken TCP (Read==0/exception)
            var rxThread = new Thread(() => RxDrainAndDetectBreak(stream))
            {
                IsBackground = true
            };
            rxThread.Start();

            // 5) Run control loop until link breaks, then restart sequence
            try
            {
                RunControlLoop(h, hidBuf, stream);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[CONTROL LOOP EXIT] {ex.GetType().Name}: {ex.Message}");
            }

            Console.WriteLine("[WATCHDOG] Link broken -> restarting ping/tcp/mavlink sequence...");
            Thread.Sleep(200);
        }
    }

    // ---------------- WATCHDOG: Ping + TCP + MAVLink ----------------

    private static void WaitPing(string host)
    {
    ...
    private static TcpClient ConnectTcpWithRetry(string host, int port)
    {
        while (true)
        {
            try
    ...
    private static void RxDrainAndDetectBreak(NetworkStream stream)
    {
        var b = new byte[4096];
        while (!_linkBroken)
        {
            try
            {
                ...
                if (n == 0)
                {
                    _linkBroken = true; // remote closed
                    return;
                }
                // drain only (no parsing)
            }
            catch
            {
                _linkBroken = true;
    ...
    private static void WaitHeartbeat(NetworkStream stream)
    {
        // Minimal MAVLink1 scan for HEARTBEAT to learn sys/comp.
        // Blocks until it sees msgid==0, or throws if stream breaks.
        var one = new byte[1];
        ...
        while (true)
        {
            ...
            if (r == 0) throw new Exception("TCP closed while waiting heartbeat");
            byte c = one[0];

            switch (state)
            {
                case 0: if (c == 0xFE) state = 1; break; // stx
                case 1: len = c; idx = 0; state = 2; break; // len
                case 2: state = 3; break; // seq
                case 3: sys = c; state = 4; break; // sysid
                case 4: comp = c; state = 5; break; // compid
                case 5:
                    msgid = c;
                    state = (len == 0) ? 7 : 6;
                    break;
    ...
    private static void RunControlLoop(IntPtr hHid, byte[] buf, NetworkStream stream)
    {
        // state
        ...
        while (!_linkBroken)
        {
            if (!ReadFile(hHid, buf, 64, out uint n, IntPtr.Zero)) continue;
            if (n < 12) continue;
            ...
            if (now >= nextCtrl)
            {
                ...
                if (_buttonsCount >= STABLE_COUNT)
                {
                    ...
                    {
                        rtlActive = false;
                        if (lastMode != null) SetModeCopter(stream, lastMode);
                    }
                ...
                if (_modeCount == STABLE_COUNT && !rtlActive && m != null && m != lastMode)
                {
        ...
        throw new Exception("Link broken (watchdog)");
    }
